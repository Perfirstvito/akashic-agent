"""Proactive prompt 构建 + context frame 渲染。

从 proactive_turn.py 抽出的协作者：装配 agent tick 的 system prompt 与本轮
runtime context frame（alerts / content / context / workspace 规则 / 用户记忆）。
"""
...
class ProactivePromptBuilder:
    """装配 proactive system prompt 与 runtime context frame。"""
    ...
    def __init__(
        ...
        *,
        ...
        workspace_rules_fn: Callable[[list[dict]], str] | None,
    ) -> None:
        self._cfg = cfg
        self._tool_deps = tool_deps
        self._workspace_context_fn = workspace_context_fn
        self._workspace_rules_fn = workspace_rules_fn
